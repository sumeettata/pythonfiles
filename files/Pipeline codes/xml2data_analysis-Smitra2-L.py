...
image_path = 'D://OneDrive//OneDrive - Tata Insights and Quants//Work//TATA Communications//Data_1//16-03-2023_ANPR//juminsir_data//Divyanshi_total_sir_7 - Copy'
xml_path = image_path 
csv_path = 'D://OneDrive//OneDrive - Tata Insights and Quants//Work//TATA Communications//Csv Files'
save_graph_path = 'D://OneDrive//OneDrive - Tata Insights and Quants//Work//TATA Communications//data_analysis'

# Input the required values to variable
Human_class = ['human','np']
Vehicle_class = ['vehicle_back','vehicle_side','vehicle_front','bicycle', 
                 'car', 'motorcycle', 'bus', 'truck','vehicle','auto']
...
df.drop(df[(df['x2'] - df['x1']) == 0].index, inplace=True)
...
# saving the plot
fig.suptitle("Ths Analysis of labelled images, The files analysed are "+str(len(df['filename'].unique())))
fig.savefig(save_graph_path+'//'+xml_path.split('//')[-1]+file_time+'.png')
