...
for path in tqdm(image_path[0:8]):
        ...
        if len(det):
            ...
            for x1, y1, x2, y2, conf, cls in reversed(det):
                if str(names[int(cls.numpy())]) == "person":
                    cv2.rectangle(image, (int(a1), int(b1)), (int(a2), int(b2)), color=(255, 255, 255), thickness=2)
                    if (x1 > a1) and (x2 > a1) and (x1 < a2) and (x2 < a2):
                        if (y1 > b1) and (y2 > b1) and (y1 < b2) and (y2 < b2):
